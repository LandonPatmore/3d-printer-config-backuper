...
# version         : 0.1
# usage		        : ./3dbackup.sh
# notes:          : Do not run this script directly, run ./3dbackup.sh from a host machine
# license         : MIT
#==============================================================================
...
GITHUB_NAME="$4"
PRIVATE_REPO="$5"
PRINTER_CONFIG_PATH="$6"
RASPBERRY_PI_USERNAME="$7"
CRON_CONFIG="$8"
REPO_NAME="3d-printer-config-files"
DESCRIPTION="This repository was generated by LandonPatmore's 3dbackup script located at: https://github.com/LandonPatmore/3dbackup"
PUBLIC_SSH_KEY=""
...
# Make sure that the .git folder ownership is changed to the user since we are using sudo to do things
sudo chown -R "$RASPBERRY_PI_USERNAME":"$RASPBERRY_PI_USERNAME" .git/
...
printf "Setting up autocommit schedule..."
CRON_PATH="/etc/cron.d/autocommit_schedule"
sudo touch "$CRON_PATH"
echo "$CRON_CONFIG /home/$RASPBERRY_PI_USERNAME/autocommit.sh $PRINTER_CONFIG_PATH" | sudo tee "$CRON_PATH" > /dev/null
sudo chmod 600 "$CRON_PATH"
printf "Set"

printf "\n\n"

printf "Your files are located at: https://github.com/%s/%s" "$GITHUB_USERNAME" "$REPO_NAME"
